...
<div class="container">
    ...
    <div class="column span-6 first last" id="related">
        ...
        <div class="column prepend-1 span-4 first last">
            <a href="<?=h( Pictures::$VIEW_PIC_URL . "?picid=" . $p['id'] ) ?>"><img
                    src="/upload/<?=h($p['filename']) ?>.128.jpg" width="128" /></a>
    ...
    <div class="column span-6 first last" id="same-upload">
        <div class="column span-6 first last">
            <h2 id="same-upload-title">Other by <a
                    href="<?= Users::$VIEW_URL ?>?userid=<?=h( $pic['user_id'] )?>"><?=h($pic['login']) ?></a></h2>
        </div>

        <?php foreach ($same as $pic) { ?>
        <div class="column prepend-1 span-4 first last" style="margin-bottom:2em;">
            <a href="<?=h( Pictures::$VIEW_PIC_URL . "?picid=" . $pic['id'] ) ?>"><img
                    src="/upload/<?=h($pic['filename']) ?>" width="128" /></a>
        </div>
        <?php }?>
    </div>
    <?php } ?>
